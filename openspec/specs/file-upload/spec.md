...
Defines the backend behavior for creating new BigQuery tables from uploaded
Excel (.xlsx) and CSV files: the stateless inspect/preview/load endpoints under
`/bigquery/upload-file/*`, server-side extraction, header sanitization and type
inference, schema approval decisions, create-only table creation with explicit
schema, and full row loading with BigQuery-native scalar values. The uploaded
file is never persisted on disk.
